...
public class EntryDetailsActivity extends AppCompatActivity
        implements DeleteConfirmationBottomSheet.DeleteConfirmationListener {

    private Transaction transaction;
    private String customerName;

    private final SimpleDateFormat sdfWithTime =
    ...
    private void populateViews() {
        ...
        Date date = new Date(transaction.getTimestamp());
        tvDate.setText(sdfWithTime.format(date));
        ...
        String smsText = "Hello " + transaction.getCustomerName() + ",\n" +
                "A transaction of " + formattedAmount + " (" + (isGave ? "You Gave" : "You Got") + ") " +
                "was recorded on " + sdfWithTime.format(date) + ".\n\nSent by MyKhata Pro.";
    ...
    private void setupListeners() {
        ...
        findViewById(R.id.btn_edit_entry_receipt).setOnClickListener(v -> {
            ...
            i.putExtra("EDIT_TRANSACTION", transaction);
            startActivity(i);
            finish();
        });
        ...
        // DELETE ENTRY
        findViewById(R.id.btn_delete_entry).setOnClickListener(v -> {
            DeleteConfirmationBottomSheet bottomSheet =
                    new DeleteConfirmationBottomSheet();
            bottomSheet.show(getSupportFragmentManager(), bottomSheet.getTag());
        });
    ...
    // ------------------------------------------------------------------
    // ⭐ MULTI-SHOP LOGIC FOR DELETE / MOVE TO RECYCLE BIN
    // ------------------------------------------------------------------
    private void moveToRecycleBinAndDelete() {
        ...
        DatabaseReference activeRef = baseRef
                .child("transactions")
                .child(transaction.getCustomerPhone())
                .child(transaction.getId());

        DatabaseReference recycleRef = baseRef
                .child("RecycleBin")
                .child(transaction.getId());
        ...
        recycleRef.setValue(transaction).addOnCompleteListener(t1 -> {
            if (t1.isSuccessful()) {
                activeRef.removeValue().addOnCompleteListener(t2 -> {
