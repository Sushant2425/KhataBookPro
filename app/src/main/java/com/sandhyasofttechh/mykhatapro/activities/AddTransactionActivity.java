...
//    }
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                new AlertDialog.Builder(this)
                        ...
                        .setPositiveButton("Yes", (dialog, which) ->
                                addCustomerToFirebase(name, finalNumber))
    ...
    // Main method to upload image to Firebase Storage
    private void uploadImageToFirebase(Uri imageUri) {
        ...
        String fileName = "transaction_" + timestamp + ".jpg";
        ...
        storageRef.putFile(imageUri)
                .addOnProgressListener(taskSnapshot -> {
                    ...
                    uploadProgressDialog.setMessage("Uploading: " + (int) progress + "%");
                })
                .addOnSuccessListener(taskSnapshot -> {
                    ...
                    storageRef.getDownloadUrl().addOnSuccessListener(uri -> {
                        ...
                        // Update UI
                        btnAttachFile.setText("Image Attached ✓");
                        btnAttachFile.setTextColor(getResources().getColor(android.R.color.holo_green_dark));
                        ...
                        Log.e("ImageUpload", "Get URL Error", e);
                    });
                })
                .addOnFailureListener(e -> {
                    if (uploadProgressDialog != null && uploadProgressDialog.isShowing()) {
    ...
    // Update your saveTransaction method to include the image URL
    private void saveTransaction() {
        String amountStr = etAmount.getText().toString().trim();
        String note = etNote.getText().toString().trim();
        String date = etDate.getText().toString();
        ...
        if (editTransaction != null) {
            ...
            layoutCustomer.setError(null);
            selectedName = customerInfo.substring(0, customerInfo.lastIndexOf(" (")).trim();
            selectedPhone = customerInfo.substring(customerInfo.lastIndexOf(" (") + 2,
        ...
        if (transactionsRef == null) {
            Toast.makeText(this, "Unable to save transaction: no shop selected",
                    Toast.LENGTH_SHORT).show();
            return;
        }

        String idToSave = (editTransaction != null && editTransaction.getId() != null)
                ? editTransaction.getId()
                : transactionsRef.child(selectedPhone).push().getKey();
        ...
        if (uploadedImageUrl != null && !uploadedImageUrl.isEmpty()) {
            transaction.setImageUrl(uploadedImageUrl);
        }

        transactionsRef.child(selectedPhone).child(idToSave).setValue(transaction)
                .addOnSuccessListener(aVoid -> {
                    if (sendSms) {
                        new GenerateStatementAndSendSmsTask(this, selectedPhone, selectedName,
                                amount, isGave, date, prefManager.getCurrentShopId()).execute();
                    } else {
                        Intent intent = new Intent(this, TransactionSuccessActivity.class);
                        intent.putExtra(TransactionSuccessActivity.EXTRA_AMOUNT, amount);
                        intent.putExtra(TransactionSuccessActivity.EXTRA_CUSTOMER, selectedName);
    ...
    // Add this method to reset image attachment
    private void resetImageAttachment() {
        selectedImageUri = null;
        uploadedImageUrl = null;
        btnAttachFile.setText("Attach File");
        btnAttachFile.setTextColor(getResources().getColor(android.R.color.black));
    }
