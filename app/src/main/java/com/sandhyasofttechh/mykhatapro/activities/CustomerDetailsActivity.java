...
public class CustomerDetailsActivity extends AppCompatActivity
    ...
    private PrefManager prefManager;
    private DatabaseReference baseRef, transactionsRef, customersRef;
    ...
    private void setupFirebasePaths() {
        ...
        if (shopId == null || shopId.isEmpty()) {
            // ⭐ NO SHOP CREATED → LOAD ROOT DATA
            baseRef = FirebaseDatabase.getInstance()
                    .getReference("Khatabook")
                    .child(emailKey);
        } else {
            // ⭐ SHOP EXISTS → LOAD DATA INSIDE SHOP
            baseRef = FirebaseDatabase.getInstance()
                    .getReference("Khatabook")
                    .child(emailKey)
                    .child("shops")
    ...
    private void setupToolbar() {
        ...
        if (getSupportActionBar() != null) {
            ...
            getSupportActionBar().setTitle(customerName);
            getSupportActionBar().setSubtitle(formatPhone(customerPhone));
        }
    }

    private String formatPhone(String phone) {
        String p = phone.replaceAll("[^0-9]", "");
        if (p.length() == 10) return p.substring(0, 5) + " " + p.substring(5);
        return phone;
    }
    ...
    @Override public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (item.getItemId() == R.id.action_edit_customer) { editCustomer(); return true; }

        if (item.getItemId() == R.id.action_share_pdf) { shareFullPdf(); return true; }
    ...
    private void editCustomer() {
        ...
        i.putExtra("CUSTOMER_PHONE", customerPhone);
    ...
    private void shareFullPdf() {
        ...
        File pdfFile = PdfGenerator.generatePdf(
                ...
                transactionList, "Full Report", 0
        );

        if (pdfFile != null) sharePdf(pdfFile);
        else Toast.makeText(this, "Failed to generate PDF", Toast.LENGTH_SHORT).show();
    }
    ...
    // --------------------------------------------------------------
    // PAGE REFRESH AFTER EDIT
    // --------------------------------------------------------------
    @Override protected void onResume() {
        super.onResume();
        String newName = getIntent().getStringExtra("UPDATED_CUSTOMER_NAME");
    ...
    // --------------------------------------------------------------
    // WHATSAPP + SMS (UNCHANGED)
    // --------------------------------------------------------------
    private void sendWhatsAppMessage() {
        String appName = getString(R.string.app_name);
        File imgFile = ImageGenerator.generateShareableImage(this, customerName, netBalance, appName);
        ...
        String phone = "91" + customerPhone;

        String msg;
        if (netBalance > 0) msg = "You have to pay ₹" + netBalance;
        else if (netBalance < 0) msg = "I have to pay you ₹" + Math.abs(netBalance);
        else msg = "Account settled.";
    ...
    private void sendSms() {
        ...
        if (netBalance > 0)
            msg = "Reminder: You need to pay ₹" + netBalance;
        else if (netBalance < 0)
            msg = "Confirmation: I need to pay you ₹" + Math.abs(netBalance);
        else
            msg = "Account settled.";
    ...
    // --------------------------------------------------------------
    // LOADING TRANSACTIONS WITH SHOP LOGIC
    // --------------------------------------------------------------
    private void loadTransactions() {
        transactionsRef.child(customerPhone)
    ...
    private void updateSummary(double gave, double got) {
        tvTotalGave.setText("₹" + gave);
        tvTotalGot.setText("₹" + got);
        ...
        tvBalance.setText("₹" + Math.abs(netBalance));
    ...
    // --------------------------------------------------------------
    private void setupRecyclerView() {
        RecyclerView rv = findViewById(R.id.recycler_customer_transactions);
    ...
    private void setupClickListeners() {
        findViewById(R.id.fab_add_specific_transaction).setOnClickListener(v -> {
            ...
            i.putExtra("edit_customer_phone", customerPhone);
            i.putExtra("edit_customer_name", customerName);
            startActivity(i);
        });
        ...
        findViewById(R.id.btn_report).setOnClickListener(v -> {
            ReportOptionsBottomSheet bs = ReportOptionsBottomSheet.newInstance(transactionList);
            bs.show(getSupportFragmentManager(), "ReportBS");
        });
    }

    // --------------------------------------------------------------
    // OPEN ENTRY DETAILS PAGE
    // --------------------------------------------------------------
    @Override
    public void onItemClick(Transaction txn) {
        Intent i = new Intent(this, EntryDetailsActivity.class);
        i.putExtra("TRANSACTION_DATA", txn);
        i.putExtra("CUSTOMER_NAME", customerName);
        startActivity(i);
    }
    ...
    // --------------------------------------------------------------
    @Override
    public void onReportGenerated(List<Transaction> list, String label) {
        if (list.isEmpty()) {
            Toast.makeText(this, "No transactions", Toast.LENGTH_SHORT).show();
            return;
        }

        File pdf = PdfGenerator.generatePdf(
                this, customerName, customerPhone, list, label, 0
        );
