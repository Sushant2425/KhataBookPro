...
public class EditTransactionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        transaction =
                (Transaction) getIntent().getSerializableExtra("EDIT_TRANSACTION");
    ...
    private void populateData() {
        initialAmount = String.format(Locale.getDefault(),
                "%.2f", transaction.getAmount());
        initialNote = transaction.getNote();
        initialDate = transaction.getDate();
    ...
    private boolean hasChanges() {
        ...
        return !currentAmount.equals(initialAmount)
    ...
    private void saveChanges() {
        ...
        transaction.setAmount(amount);
        transaction.setNote(
                Objects.requireNonNull(etNote.getText()).toString().trim());
        transaction.setDate(
    ...
    // ------------------------------------------------------------------
    // ⭐ MULTI-SHOP FIREBASE SAVE LOGIC
    // ------------------------------------------------------------------
    private void saveToFirebase(Transaction txn) {
        ...
        DatabaseReference dbRef = baseRef
                .child("transactions")
                .child(txn.getCustomerPhone())
                .child(txn.getId());

        dbRef.setValue(txn).addOnCompleteListener(task -> {
