    ...
    private void loginUser() {
        ...
        mAuth.signInWithEmailAndPassword(email, password).addOnCompleteListener(task -> {
            ...
            if (task.isSuccessful()) {
                FirebaseUser user = mAuth.getCurrentUser();
                if (user != null && user.isEmailVerified()) {
                    loadUserProfileAndShops(email);
                } else {
                    Toast.makeText(LoginActivity.this, "Please verify your email first", Toast.LENGTH_LONG).show();
                    FirebaseAuth.getInstance().signOut();
                }
            } else {
                Toast.makeText(LoginActivity.this, "Login failed: " + (task.getException()!=null?task.getException().getMessage():""), Toast.LENGTH_LONG).show();
            }
        });
    }

    private void loadUserProfileAndShops(String email) {
        String emailKey = email.replace(".", ",");
        DatabaseReference userRef = FirebaseDatabase.getInstance().getReference("Khatabook").child(emailKey).child("profile");

        // read profile then read shops
        userRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot snapshot) {
                if (!snapshot.exists()) {
                ...
                String userEmail = snapshot.child("email").getValue(String.class);
                String fullName;
                if (snapshot.child("name").exists()) fullName = snapshot.child("name").getValue(String.class);
                else {
                    fullName = "";
                }
                ...
                prefManager.setLogin(true);
                prefManager.setUserEmail(userEmail);
                prefManager.setUserName(fullName != null ? fullName : "");

                // Now load shops
                DatabaseReference shopsRef = FirebaseDatabase.getInstance()
                        .getReference("Khatabook")
                        .child(emailKey)
                        .child("shops");
                ...
                shopsRef.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot shopSnap) {
                        if (!shopSnap.exists()) {
                            ...
                            if (shopId == null) {
                                // proceed to main anyway
                                startMain();
                                return;
                            }
                            String createdAt = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", Locale.getDefault()).format(new Date());
                            String defaultShopName = (fullName != null && !fullName.isEmpty()) ? (fullName + " Shop") : "My Shop";
                            Shop shop = new Shop(shopId, defaultShopName, createdAt);
                            shopsRef.child(shopId).setValue(shop).addOnSuccessListener(aVoid -> {
                                prefManager.setCurrentShopId(shopId);
                                prefManager.setCurrentShopName(defaultShopName);
                                startMain();
                            }).addOnFailureListener(e -> {
                                // if failed to create, continue anyway
                                startMain();
                            });
                        } else {
                            // pick previously selected shop (if exists) otherwise pick the first shop
                            String currentShopId = prefManager.getCurrentShopId();
                            if (currentShopId != null && !currentShopId.isEmpty() && shopSnap.hasChild(currentShopId)) {
                                // keep existing
                                startMain();
                            } else {
                                // choose first shop from list
                                for (DataSnapshot s : shopSnap.getChildren()) {
                                    Shop sh = s.getValue(Shop.class);
                                    if (sh != null) {
                                        prefManager.setCurrentShopId(sh.getShopId());
                                        prefManager.setCurrentShopName(sh.getShopName());
                                        break;
                                    } else {
                                        String id = s.getKey();
                                        String name = s.child("shopName").getValue(String.class);
                                        prefManager.setCurrentShopId(id);
                                        prefManager.setCurrentShopName(name != null ? name : "My Shop");
                                        break;
                                    }
                                }
                                startMain();
                    ...
                    public void onCancelled(@NonNull DatabaseError error) {
                        startMain(); // proceed even if shops cannot be loaded
                    }
                });
