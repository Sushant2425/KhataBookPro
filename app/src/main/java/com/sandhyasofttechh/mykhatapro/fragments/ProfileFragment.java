...
public class ProfileFragment extends Fragment {
    ...
    private final ActivityResultLauncher<Uri> cameraLauncher =
            registerForActivityResult(new ActivityResultContracts.TakePicture(),
                    success -> {
                        if (success && logoUri != null) handleImageResult(logoUri);
                        else Toast.makeText(getContext(), "Failed to capture", Toast.LENGTH_SHORT).show();
                    });
    ...
    private final ActivityResultLauncher<String[]> permissionLauncher =
            registerForActivityResult(new ActivityResultContracts.RequestMultiplePermissions(),
                    permissions -> {
                        boolean granted =
                                Boolean.TRUE.equals(permissions.get(Manifest.permission.CAMERA)) &&
                                        (Boolean.TRUE.equals(permissions.get(Manifest.permission.READ_MEDIA_IMAGES))
                                                || Boolean.TRUE.equals(permissions.get(Manifest.permission.READ_EXTERNAL_STORAGE)));

                        if (granted) openImageChooser();
                        else Toast.makeText(getContext(), "Permissions required", Toast.LENGTH_SHORT).show();
                    });
    ...
    public View onCreateView(
        ...
        // Image Click
        View.OnClickListener photoClick = v -> requestPermission();
        ivProfileLogo.setOnClickListener(photoClick);
        ivCameraIcon.setOnClickListener(photoClick);
        ...
        if (shopId != null && !shopId.isEmpty()) {
            ...
            logoRef = FirebaseStorage.getInstance()
                    .getReference("shop_profile_logos")
                    .child(emailKey + "_" + shopId + ".jpg");
    ...
    private void openCamera() {
        ...
        logoUri = FileProvider.getUriForFile(
                requireContext(),
                "com.sandhyasofttechh.mykhatapro.fileprovider",
                photoFile
        );
    ...
    // -------------------------------------------------------------------------
    // LOAD PROFILE (ROOT OR SHOP)
    private void loadProfileData() {
        profileRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override public void onDataChange(@NonNull DataSnapshot snap) {
                if (!isAdded()) return;

                etName.setText(getString(snap, "name"));
                etBusinessName.setText(getString(snap, "businessName"));
                etMobile.setText(getString(snap, "mobile"));
                etAddress.setText(getString(snap, "address"));
                etGstin.setText(getString(snap, "gstin"));
                etEmail.setText(getString(snap, "email", mAuth.getCurrentUser().getEmail()));

                String logo = snap.child("logoUrl").getValue(String.class);

                Glide.with(ProfileFragment.this)
                        .load(logo != null && !logo.isEmpty() ? logo : R.drawable.img)
                        .placeholder(R.drawable.img)
                        .error(R.drawable.img)
                        .circleCrop()
                        .into(ivProfileLogo);
            ...
            @Override public void onCancelled(@NonNull DatabaseError error) {
                if (isAdded()) Toast.makeText(getContext(), "Load failed", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void saveProfile() {
        ...
        if (logoUri != null) uploadLogoAndSave(name);
        else saveData(null, name);
    }

    private void uploadLogoAndSave(String name) {
        logoRef.putFile(logoUri)
                .addOnSuccessListener(task -> logoRef.getDownloadUrl()
                        .addOnSuccessListener(uri -> saveData(uri.toString(), name)))
                .addOnFailureListener(e -> {
                    progressBarProfile.setVisibility(View.GONE);
    ...
    private void saveData(String uploadedLogoUrl, String name) {
        ...
        data.put("gstin", etGstin.getText().toString().trim());
        data.put("email", etEmail.getText().toString().trim());
        data.put("updatedAt", System.currentTimeMillis());
        data.put("status", true);

        if (uploadedLogoUrl != null)
            data.put("logoUrl", uploadedLogoUrl);
